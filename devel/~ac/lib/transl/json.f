\ парсер JSON:
\ JS-объекты представляются в виде словарей Форта
\ JS-массивы представляются в виде словарей Форта со строками нулевой длины в качестве имён
\ JS-строки представляются в виде str5-строк Форта
\ JS-числа представляются в виде str5-строк Форта
\ JS-null/bool представляются в виде чисел Форта
\ К каждому из этих значений добавляется константа-тип, т.е. каждое значение JS
\ выглядит в форте как "x t", где x - объект Форта (как описано выше),
\ а t - константа js_* (описано ниже)
\ В словарь они компилируются как "C, ," в тело CREATE-статьи.
...
: JsonArray ( -- wid t )
  ...
  \ значит дальше идёт список значений
