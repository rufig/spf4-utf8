...
control table splitter-t
  item -grid1	    \ первая сетка
  item -grid2	    \ вторая сетка
  item -isvert	    \ вертикальный разделитель?
  item -ratio   set \ Размер первой сетки в % от общего размера разделителя
  item -spcur       \ Текущее положение разделителя (в точках)
  item -spmin	    \ крайнее левое значение (в точках)
  item -spmax	    \ крайнее правое значение (в точках)
  item -size2       \ минимальный размер второй сетки в направлении разделителя
  item -spdrag      \ флажок: режим перемещения
endtable
