\ Работа со строками
\ Написана под сильным влиянием ~ac\lib\str.f
\ Описание читайте там же
\ Все строки хранятся в стеке строк
\ Работа с ним аналогично R> >R только S> >S S@ ...
...
: S> ( -- addr u )
\ Переносит строку с вершины стека строк в PAD (макс. размер 1024)
   S@ DUP >R
   PAD SWAP CMOVE
   0 PAD R@ + C!
   PAD R>
...
: .SS
\ Распечатывает содержимое стека строк. Аналог .S
...
: StrCat ( c-addr u -- )
\ Добавить строку c-addr к строке на стеке строк
...
: %SYM ( с -- addr u1 )
\ Добавить к разбираемой строке символ, код которого лежит на стеке
  _'' ! _'' 1
  CELL sp_save +!
;
...
: %U ( u -- addr u1 )
\ Добавить к разбираемой строке число со стека данных в DECIMAL без знака
  BASE @ >R
  DECIMAL
  0 <# #S #>
  R> BASE !
...
: %D ( n -- addr u1 ) 
\ Добавить к разбираемой строке число со стека данных в DECIMAL со знаком
  BASE @ >R
  DECIMAL DUP >R ABS
  0 <# #S R> SIGN #>
  R> BASE !
...
: %H ( u -- addr u1 )
\ Добавить к разбираемой строке число со стека данных в HEX
  BASE @ >R
  HEX
  0 <# #S #>
  R> BASE !
...
: %S ( addr u -- addr u )
\ Добавить к разбираемой строке строку addr u
  2 CELLS sp_save +!
;
...
: STR-EVAL ( addr u S: s -- S: s )
   ...
   SP@ sp_save @ - DUP
     ...
     \ =8 - ничего не делать
   4 =
   IF DROP 0 <# #S #>  \ преобразовать в текущей системе счисления
      StrCat
