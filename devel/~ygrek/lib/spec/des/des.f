...
\ после выполнения операции результат будет помещён обратно в data-addr
\ Ключ тоже должен быть 64 бит, но реально используются только 56 из них
\ Остальные применяются для контроля чётности но в этой реализации не проверяются
\ т.е. key-addr должен указывать на 8 байт ключа
\ содержимое key-addr не меняется
