...
\ Область для отрисовки с точки зрения пользователя
CLASS CGLCanvas

\ действия при изменении размера (переустановить viewport, projection)
: :resize ( width height -- ) 2DROP ;

\ действия при отрисовке каждого кадра (modelview)
: :display ;
...
\ Пример как использовать canvas
\ Сгодится для простых случаев
